...
class Data extends AbstractHelper
{
	...
	private function getProductImages($product, $_config, $prefix = 'img')
	{
				...
				switch ($type) {
					case 'product_image':
						...
						if (file_exists($_mediaCheck) || @getimagesize($_mediaCheck) !== false ) {
							array_push($images_path, $_mediaCheck);
						}
						break;
		...
		$placeholder = $_config[$prefix . '_replacement'];
		$_placeholder = ($placeholder != '' && strpos($placeholder, 'http') !== false) ? $placeholder : $this->_getBaseDirMedia(). $placeholder;
		if (count($images_path) == 0 && (@getimagesize($_placeholder) !== false) || (count($images_path) == 0 && $_placeholder != "")) {
			array_push($images_path, $_placeholder);
		}
		return $images_path;
	}
